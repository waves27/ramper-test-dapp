...
const initialize = async () => {
  try {
    ...
    ethersProvider = new ethers.providers.Web3Provider(window.ramper2, 'any');
  ...
  const isMetaMaskInstalled = () => window.ramper2 ? true : false;
  ...
  const onClickConnect = async () => {
    try {
      const newAccounts = await window.ramper2.request({
  ...
  const updateButtons = () => {
    console.log('watch', {
      isRamperInstalled: isMetaMaskInstalled(),
      isRamperConnected: isMetaMaskConnected(),
      accounts,
    });
  ...
  const initializeAccountButtons = () => {
    ...
    sendFailingButton.onclick = async () => {
      try {
        const result = await window.ramper2.request({
    ...
    watchNFTsButton.onclick = async () => {
      ...
      let watchNftsResult;
      try {
        watchNftsResult = await window.ramper2.sendAsync(
    ...
    watchNFTButton.onclick = async () => {
      let watchNftsResult;
      try {
        watchNftsResult = await window.ramper2.request({
          method: 'wallet_watchAsset',
          params: {
            type: 'ERC721',
            options: {
    ...
    requestPermissionsButton.onclick = async () => {
      try {
        const permissionsArray = await window.ramper2.request({
    ...
    getAccountsButton.onclick = async () => {
      try {
        const _accounts = await window.ramper2.request({
          method: 'eth_accounts',
        });
    ...
    getEncryptionKeyButton.onclick = async () => {
      try {
        encryptionKeyDisplay.innerText = await window.ramper2.request({
    ...
    decryptButton.onclick = async () => {
      try {
        cleartextDisplay.innerText = await window.ramper2.request({
          method: 'eth_decrypt',
          params: [ciphertextDisplay.innerText, window.ramper2.selectedAddress],
        });
      } catch (error) {
  ...
  ethSign.onclick = async () => {
    try {
      ...
      const ethResult = await window.ramper2.request({
        method: 'eth_sign',
  ...
  personalSign.onclick = async () => {
    ...
    try {
      ...
      const sign = await window.ramper2.request({
  ...
  personalSignVerify.onclick = async () => {
    ...
    try {
      ...
      const ecRecoverAddr = await window.ramper2.request({
  ...
  signPermit.onclick = async () => {
    ...
    try {
      sign = await window.ramper2.request({
        method: 'eth_signTypedData_v4',
        params: [from, JSON.stringify(msgParams)],
      });
  ...
  async function getNetworkAndChainId() {
    try {
      const chainId = await window.ramper2.request({
        method: 'eth_chainId',
      });
      handleNewChain(chainId);

      const networkId = await window.ramper2.request({
        method: 'net_version',
      });
      handleNewNetwork(networkId);

      const block = await window.ramper2.request({
  ...
  if (isMetaMaskInstalled()) {
    window.ramper2.autoRefreshOnNetworkChange = false;
    getNetworkAndChainId();

    window.ramper2.autoRefreshOnNetworkChange = false;
    getNetworkAndChainId();

    window.ramper2.on('chainChanged', (chain) => {
    ...
    window.ramper2.on('chainChanged', handleNewNetwork);
    window.ramper2.on('accountsChanged', (newAccounts) => {
      ethereum
        .request({
          method: 'eth_getBlockByNumber',
          params: ['latest', false],
